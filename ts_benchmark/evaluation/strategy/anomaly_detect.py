...
class AnomalyDetect(Strategy):
    ...
    def execute(self, series_name: str, model_factory: ModelFactory) -> Any:
        ...
        try:
            self.model = model
            logger.info("Starting anomaly evaluation for series: %s", series_name)
            train_data, train_label, test_data, test_label = self.split_data(
                series_name
            )
            logger.info(
                "Split series %s: train_shape=%s test_shape=%s",
                series_name,
                train_data.shape,
                test_data.shape,
            )
            ...
            logger.info(
                "Fitting %s on series %s", model_factory.model_name, series_name
            )
            ...
            logger.info(
                "Finished fit for %s on series %s in %.2fs",
                model_factory.model_name,
                series_name,
                end_fit_time - start_fit_time,
            )
            logger.info(
                "Running inference for %s on series %s",
                model_factory.model_name,
                series_name,
            )
            ...
            logger.info(
                "Finished inference for %s on series %s in %.2fs",
                model_factory.model_name,
                series_name,
            ...
            logger.exception("Anomaly evaluation failed for series: %s", series_name)
            single_series_results_list = [
                self.get_default_result(**{FieldNames.LOG_INFO: log})
            ]
