...
def eval_model(
    ...
    for series_idx, series_name in enumerate(
        tqdm.tqdm(series_list, desc=f"scheduling {model_factory.model_name}"),
        start=1,
    ):
        # TODO: refactor data model to optimize communication cost in parallel mode
        logger.info(
            "Scheduling/evaluating %s series %d/%d: %s",
        ...
        logger.info(
            "Scheduled/evaluated %s series %d/%d: %s",
            model_factory.model_name,
            series_idx,
            total_series,
            series_name,
