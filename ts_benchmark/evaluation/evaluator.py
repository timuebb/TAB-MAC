...
class Evaluator:
    ...
    def evaluate_with_log(
        ...
        **kwargs,
        ...
        log_info = ""
        for m in self.metric_funcs:
            metric_name = getattr(m, "__name__", repr(m))
            start_time = time.time()
            try:
                logger.info(
                    "Starting metric %s: actual_len=%d predicted_len=%d",
                    metric_name,
                    len(actual),
                    len(predicted),
                )
                ...
                logger.info(
                    "Finished metric %s in %.2fs",
                    metric_name,
                    time.time() - start_time,
                )
            except Exception as e:
                evaluate_result.append(np.nan)
                logger.exception("Failed metric %s", metric_name)
