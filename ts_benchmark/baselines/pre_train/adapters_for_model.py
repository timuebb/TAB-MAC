...
class PreTrainAdapter(ModelBase):
    ...
    @staticmethod
    def _select_tensor_output(output):
        if isinstance(output, dict):
            for key in ("outputs_time", "output", "outputs", "reconstruction"):
                if key in output:
                    return output[key]
    ...
    def _align_anomaly_tensors(self, output, target):
        output = self._select_tensor_output(output)

        if output.ndim != target.ndim:
            return output, target

        if output.shape[-1] != target.shape[-1]:
            feature_count = min(output.shape[-1], target.shape[-1])
            output = output[..., :feature_count]
            target = target[..., :feature_count]

        if output.shape[1] != target.shape[1]:
            window = min(output.shape[1], target.shape[1])
    ...
    def detect_validate(self, valid_data_loader, criterion):
        ...
        for input, _ in valid_data_loader:
            ...
            output, true = self._align_anomaly_tensors(output, input)
    ...
    def detect_fit(self, train_data: pd.DataFrame, test_data: pd.DataFrame):
        ...
        config = self.config
        train_ratio_in_tv = getattr(config, "train_ratio_in_tv", 0.8)
        train_data_value, valid_data = train_val_split(train_data, train_ratio_in_tv, None)
        ...
        if config.is_train:
            ...
            for epoch in range(config.num_epochs):
                ...
                for i, (input, target) in enumerate(self.train_data_loader):
                    ...
                    output, loss_target = self._align_anomaly_tensors(output, input)
    ...
    def detect_score(self, test: pd.DataFrame) -> np.ndarray:
        ...
        for i, (batch_x, batch_y) in enumerate(self.thre_loader):
            ...
            outputs, score_target = self._align_anomaly_tensors(outputs, batch_x)
            score = torch.mean(self.anomaly_criterion(score_target, outputs), dim=-1)
    ...
    def detect_label(self, test: pd.DataFrame) -> np.ndarray:
        ...
        for i, (batch_x, batch_y) in enumerate(self.thre_loader):
            ...
            outputs = self.model(batch_x)
            outputs, score_target = self._align_anomaly_tensors(outputs, batch_x)
            score = torch.mean(self.anomaly_criterion(score_target, outputs), dim=-1) #(6, 672) (B, L) 对变量平均了
            score = score.detach().cpu().numpy()
