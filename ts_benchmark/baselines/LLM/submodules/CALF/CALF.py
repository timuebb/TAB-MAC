...
class Encoder_PCA(nn.Module):
    ...
    def forward(self, x):
        ...
        q = x
        k = v = self.word_embedding
        x, _ = self.cross_attention(q, k, v)

        return x_time, x


def load_gpt2_with_eager_attention(path):
    kwargs = {
        "output_attentions": True,
        "output_hidden_states": True,
    }
    try:
        return AccustumGPT2Model.from_pretrained(
            path,
            attn_implementation="eager",
            **kwargs,
        )
    except TypeError:
        return AccustumGPT2Model.from_pretrained(path, **kwargs)


def load_word_embedding(path, device):
    try:
    ...
    if isinstance(loaded, torch.Tensor):
        return loaded.clone().detach().to(device=device)
    return torch.as_tensor(loaded, device=device)
...
class Model(nn.Module):
    def __init__(self, configs, device):
        ...
        self.gpt2 = load_gpt2_with_eager_attention('ts_benchmark/baselines/LLM/checkpoints/gpt2')  # loads a pretrained GPT-2 base model
        self.gpt2_text = load_gpt2_with_eager_attention('ts_benchmark/baselines/LLM/checkpoints/gpt2')  # loads a pretrained GPT-2 base model

        self.gpt2.h = self.gpt2.h[:configs.gpt_layers]
        self.gpt2_text.h = self.gpt2_text.h[:configs.gpt_layers]
        self.gpt2 = get_peft_model(self.gpt2, peft_config)
        
        word_embedding = load_word_embedding(configs.word_embedding_path, device)
