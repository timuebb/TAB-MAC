...
class AnomalyTransformer:
    ...
    def detect_fit(self, train_data: pd.DataFrame, test_data: pd.DataFrame):
        ...
        train_data_value, valid_data = train_val_split(
            train_data, 0.8, self.config.win_size
        )
        ...
        if train_steps <= 0:
            raise ValueError(
                f"Not enough train rows ({len(train_data_value)}) for win_size={self.config.win_size}"
            )
        if valid_steps <= 0:
            raise ValueError(
                f"Not enough validation rows ({len(valid_data)}) for win_size={self.config.win_size}"
            )
        ...
        logger.info(
            "AnomalyTransformer fit setup: device=%s train_shape=%s valid_shape=%s "
            "train_steps=%d valid_steps=%d batch_size=%d win_size=%d step=%d "
            "d_model=%d d_ff=%d n_heads=%d e_layers=%d params=%d",
            self.device,
        ...
        for epoch in range(self.config.num_epochs):
            ...
            for i, (input_data, labels) in enumerate(self.train_loader):
                ...
                if should_log_batch:
                    logger.info(
                        "AnomalyTransformer epoch %d/%d batch %d/%d start: input_shape=%s",
                        epoch + 1,
                ...
                if should_log_batch:
                    logger.info(
                        "AnomalyTransformer epoch %d/%d batch %d/%d done: "
                        "rec_loss=%.7f loss1=%.7f loss2=%.7f",
                        epoch + 1,
                        self.config.num_epochs,
                        i + 1,
                        train_steps,
            ...
            logger.info(
                "AnomalyTransformer epoch %d/%d validation start: valid_steps=%d",
                epoch + 1,
                self.config.num_epochs,
                valid_steps,
            )
