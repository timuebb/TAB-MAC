...
class hp_filter(nn.Module):
    ...
    def forward(self, x):
        ...
        device = x.device
        N = x.shape[1]
        D1 = D_matrix(N).to(device=device, dtype=x.dtype)
        D2 = D_matrix(N - 1).to(device=device, dtype=x.dtype)
        D = torch.mm(D2, D1)

        g = torch.matmul(torch.inverse(torch.eye(N, device=device, dtype=x.dtype) + self.lamb * torch.mm(D.T, D)), x)
        res = x - g
        g = g.permute(0, 2, 1)
